...
# BGJ: Adapted to avoid overlap in top and bottom of peripherical labels
...
plot_radarchart_several_taxaLevels <- function(physeq_pred_sub,suffix0){
  for(tax_rank in c("Phylum","Class")){
    ...
    pdf(paste('../Results/NovelEcosystems/radarChart_otus_predFromDomain_novelSamples_',suffix,'.pdf',sep=''))
    ...
    dev.off()
    embed_fonts(paste('../Results/NovelEcosystems/radarChart_otus_predFromDomain_novelSamples_',suffix,'.pdf',sep=''))
  } # end-for
} # end-function
###
...
physeq_pred = build_physeq_object('../Results/otus_predFromDomain_novelSamples.tsv','../Datasets/metadata_novel_samples_only3envFeatures.csv',file_tax)
